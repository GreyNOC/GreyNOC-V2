...
class GreyNOC(tk.Tk):
    ...
    def _draw_map_on(self,c,W,H,zoom,px,py):
        ...
        # ── Clean lat/lon grid — very subtle dark red lines, no dash clutter ──
        for lon in range(-180,181,30):
            x,_=proj(lon,0)
            c.create_line(x,0,x,H,fill=MAP_GRID,width=1,dash=(1,6))
        for lat in range(-60,91,30):
            _,y=proj(0,lat)
            c.create_line(0,y,W,y,fill=MAP_GRID,width=1,dash=(1,6))

        # ── Equator & prime meridian slightly brighter ──
        _,eq_y=proj(0,0)
        c.create_line(0,eq_y,W,eq_y,fill="#3a0010",width=1)
        pm_x,_=proj(0,0)
        c.create_line(pm_x,0,pm_x,H,fill="#3a0010",width=1)

        # ── Continent outlines — WIRE ONLY, no fill ──
        for name, pts in CONTINENTS.items():
            coords = []
            for lon, lat in pts:
            ...
            if len(coords) >= 6:
                try:
                    c.create_polygon(coords,
                                     fill="",           # transparent fill
                                     outline="#ff3355", # bright red wire
                                     width=2)
                except:
                    pass
        ...
        for i,(sx,sy,dx,dy,age,col) in enumerate(self.map_lines):
            ...
            mx=(sx2+dx2)/2; my=min(sy2,dy2)-24*fade
            # White arc line with protocol color tint
            line_col=MAP_WHITE if fade>0.7 else col
            c.create_line(sx2,sy2,mx,my,dx2,dy2,
                          fill=line_col,width=max(1,int(2*fade)),smooth=True)
            # Animated dot
            t=(age%18)/18.0
        ...
        # ── Host nodes — white labels, protocol-colored rings ──
        ...
        for ip,node in list(self.map_nodes.items()):
            ...
            # IP label — always white for readability
            if zoom>1.5 or pkts>10:
                octets=ip.split(".")
                lbl=ip if zoom>2.5 else f"{octets[0]}.···.{octets[-1]}"
                # Small black shadow for contrast
                c.create_text(x+rc+4,y-3,text=lbl,fill="#000000",
                              font=FM,anchor="w")
                c.create_text(x+rc+3,y-4,text=lbl,fill=MAP_WHITE,
                              font=FM,anchor="w")

            # Cross-hair for very active hosts
            if pkts>25:
                chl=ro+5
        ...
        # ── Corner HUD — white labels ──
        # Thin dark backing rectangles for legibility
        ...
        c.create_rectangle(0,0,W,H,outline=C["map_dim"],width=3)
